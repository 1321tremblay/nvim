...
vim.api.nvim_create_autocmd("BufWritePre", {
  ...
  callback = function()
    ...
    for line_num = 0, vim.api.nvim_buf_line_count(bufnr) - 1 do
      ...
      if line:match "Last updated on:" then
        local new_date = os.date "Last updated on: %Y/%m/%d %H:%M"
...
require("lazy").setup({
  ...
  },
  {
    ...
    config = function()
      ...
      -- Setup lsp manually in here on Nixos
      ...
      lspconfig.astro.setup {
        cmd = { "astro-ls", "--stdio" },
        filetypes = { "astro" },
        root_dir = lspconfig.util.root_pattern("astro.config.mjs", "astro.config.ts"),
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      -- set statusline bg color to transparent
